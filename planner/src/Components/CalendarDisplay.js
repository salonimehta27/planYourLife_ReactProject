...
function CalendarDisplay() {
	...
	const calendarDisplay = displayCalendar.map((d) => (
		<CalendarCard
			id={d.id}
			key={d.id}
			url={d.url}
	...
	return (
		<div>
			<form onSubmit={handleAddCalendar}>
				<input
					type="url"
					placeholder="Enter Url of your Google Calendar"
					value={googleCalendar}
					onChange={(e) => setGoogleCalendar(e.target.value)}
				></input>
				<button style={{ marginBottom: "3px" }}>Add your Calendar</button>
			</form>
			<h5 onClick={() => setNote(!note)} style={{ color: "#8F25FA" }}>
				{note
					? "Note: This Calendar is not Editable, this is just to view your Google Calendar at a glance & you can add or delete multiple calendars, for the purposes of demo this Calendar shown below will not permanently delete"
					: "Click to See note"}
			</h5>
			<br></br>
			{calendarDisplay}
